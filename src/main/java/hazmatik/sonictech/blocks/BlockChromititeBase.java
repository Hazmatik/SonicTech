...
public class BlockChromititeBase extends Block implements IHasModel, IMetaName
{
	public static final PropertyEnum<RockType> VARIANT = PropertyEnum.create("type", RockType.class);
	
	private String name;
	
	public BlockChromititeBase(String name, float hardness,  String toolClass, int level)
	{
		super(Material.ROCK);
	...
	public void registerModels() 
	{
		for(int i = 0; i < RockType.METADATA_LOOKUP.length; i++)
		{
			SonicTech.proxy.registerVariantRenderer(Item.getItemFromBlock(this), i, "chromitite_" + RockType.values()[i].getName(), "inventory");
		}
	}
