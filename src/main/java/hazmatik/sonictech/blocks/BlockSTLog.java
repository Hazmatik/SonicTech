...
public class BlockSTLog extends BlockLog implements IHasModel, IMetaName
{
	public static final PropertyEnum<WoodType> VARIANT = PropertyEnum.create("type", WoodType.class, new Predicate<WoodType>()
			{
				...
				{
					return apply.getMetadata() < 5;
				}
			});
	
	private String name;
	
	public BlockSTLog(String name)
	{
		setUnlocalizedName(name);
		setRegistryName(name);
		setSoundType(SoundType.WOOD);
		setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, WoodType.MAGNOLIA).withProperty(LOG_AXIS, EnumAxis.Y));
