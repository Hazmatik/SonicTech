...
public class BlockSTLeafB extends BlockLeaves implements IHasModel, IMetaName
{
	...
	public BlockSTLeafB(String name)
	{
		setUnlocalizedName(name);
		setRegistryName(name);
		setSoundType(SoundType.PLANT);
		setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, WoodType.HICKORY).withProperty(CHECK_DECAY, Boolean.valueOf(true)).withProperty(DECAYABLE, Boolean.valueOf(true)));
	...
	{
		return this.getDefaultState().withProperty(VARIANT, WoodType.byMetadata(meta % 2));
	}
	...
	public void registerModels() 
	{
		for(int i = 0; i < WoodType.METADATA_LOOKUP.length; i++)
		{
			SonicTech.proxy.registerVariantRenderer(Item.getItemFromBlock(this), i, "leaves_b_" + WoodType.values()[i].getName(), "inventory");
		}
	}
